...
function setup(){
   ...
   acabouSprite = createSprite(width/2,height/2-50);
...
function draw(){
   ...
   if(estadodojogo === PLAY){
      ...
      //Faz o bowser pular com o espaço no computador e tocando na tela do celular
      if(keyDown("space") && bowser.y >= height-120 || touches.length > 0 && bowser.y >= height-120){
         bowser.velocityY = -12;
         somDoPulo.play();
         touches = [];
      }
   ...
   //Define as configurações para o estado GAMEOVER
      ...
      //Se o mouse aperta o botão de restart ou toca na tela do celular, o jogo recomeça
      if(mousePressedOver(restartSprite) || touches.length > 0){
         resetar();
         touches = [];
      }
   }
   ...
   fill("black");
   text("Pontuação: "+pontos,50,height/2);
}
...
function criarNuvens(){
   ...
   //Faz as nuvens terem posições aleatórias
...
function criarCactos(){
  ...
  if(frameCount % 60 === 0){
     ...
     var bowsermau = createSprite(width,height-95,10,40);
